...
func GetSharedInput(maxRead time.Duration) (*InterruptReader, chan os.Signal) {
...
func (ir *InterruptReader) RawMode() error {
	...
	fd := ir.In.Fd()
	var err error
	ir.st, err = platformMakeRaw(safecast.MustConv[int](fd))
	ir.mu.Unlock()
	if err != nil {
...
func (ir *InterruptReader) NormalMode() error {
	...
	err := platformRestore(safecast.MustConv[int](ir.In.Fd()), ir.st)
	ir.st = nil
	ir.mu.Unlock()
	return err
}
